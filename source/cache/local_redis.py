...
class RedisCache(CacheBase):
    def __init__(self, config):
        ...
        self.redis_host = config.get("redisHost", "redis")
        self.redis_port = config.get("redisPort", 6379)
        self.redis_url = f"redis://{self.redis_host}:{self.redis_port}"
        self._redis_client = None
        self.media_expiration = int(config.get("redisExpiration", 604800))  # 7 days by default
    ...
    def get_list(self, key: str) -> List[Any]:
        """
        Retrieve a list from Redis cache.
        """
        result = self.get(key)
    ...
    def get_or_set(self, func: callable, *args, **kwargs) -> Any:
        """
        Get the result from cache if it exists, otherwise execute the function and cache the result.
        """
    ...
    def _execute_func(self, func, *args, **kwargs):
        """
        Execute the function, handling both synchronous and asynchronous functions.
        """
        self.logger.debug(f"Executing function: {func.__name__}")
        start_time = time.time()

        # Vérifier si la fonction attend des arguments
        ...
        # Préparer les arguments pour l'appel de la fonction
        call_args = {}
    ...
    def get_ttl(self, key: str) -> int:
        """
        Get the Time To Live (TTL) for a key in Redis cache.
        """
        try:
            client = self.get_redis_client()
    ...
    def update_expiration(self, key: str, expiration: int) -> bool:
        """
        Update the expiration time for a key in Redis cache.
        """
        try:
            client = self.get_redis_client()
